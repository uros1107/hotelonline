...
class DB extends PDO
{
    public function last_row_count()
    {
        ...
        console.log("here" + $this->query('SELECT FOUND_ROWS()'));
    }
}
/**
 * autoload()
...
function db_prepareUpdate($db, $table, $data)
{
    ...
    if(isset($data['lang']) && db_column_exists($db, $table, 'lang')) $query .= ' AND lang = '.$db->quote($data['lang']);
    $result = $db->prepare($query);
    foreach($list_cols as $i => $column){
        if(array_key_exists($column, $data)){
            $col_type = db_column_type($db, $table, $column);
...
{
    $result = $db->query('SELECT * FROM information_schema.COLUMNS WHERE TABLE_NAME = '.$db->quote($table).' AND TABLE_SCHEMA = '.$db->quote(DB_NAME).' AND COLUMN_NAME = '.$db->quote($column));
    if($result !== false && $db->last_row_count() > 0)
        return true;
    else
        return false;
...
function db_getFieldValue($db, $table, $col, $id, $lang = 0)
{
    ...
    if($lang > 0 && db_column_exists($db, $table, 'lang')) $query .= ' AND lang = '.$db->quote($lang);
...
/***********************************************************************
 * mb_substr_replace() equivalement of php native function substr_replace() for multybytes strings
 *
...
function highlight($haystack, $needle, $startTag = '<b>', $endTag = '</b>')
{
    ...
    foreach($needles as $needle){
        $needle = mb_strtoupper(cleanAccent($needle));
        $offset = 0;
        $len = mb_strlen($needle);
        while(($pos = mb_strpos($haystack_format, $needle, $offset)) !== false){
            $offset = $pos+$startTagLen+$len;
            $haystack = mb_substr_replace($haystack, $startTag, $pos, 0);
            $haystack = mb_substr_replace($haystack, $endTag, $pos+$startTagLen+$len, 0);
            
            $haystack_format = mb_substr_replace($haystack_format, $startTag, $pos, 0);
            $haystack_format = mb_substr_replace($haystack_format, $endTag, $pos+$startTagLen+$len, 0);
            $offset += $endTagLen;
        }
    }
    return $haystack;
...
/***********************************************************************
 * text_format() formats a string witout special characters
 *
 * @param string $str       string to format
...
function sendMail($recipient_email, $recipient_name, $subject, $content, $reply_email = '', $reply_name = '', $from_email = '', $from_name = '', $attachements = array(), $emails_copy = '')
{
...
/***********************************************************************
 * getMail() gets the HTML content of an email
 * 
 * @param PDOStatement $db  database connection ressource
 ...
 * @return string
 */
function getMail($db, $name, $vars = array()){
    
    $result = $db->query('SELECT * FROM pm_email_content WHERE name = '.$db->quote($name).' AND lang = '.LANG_ID);
    if($result !== false && $db->last_row_count() > 0){
        $row = $result->fetch();
        $content = $row['content'];
 ...
 * @return float
 */
function formatPrice($price, $currency = CURRENCY_SIGN)
{
    $format = (CURRENCY_POS == 'after') ? '%!n '.$currency : $currency.' %!n';
    if(function_exists('money_format'))
        return preg_replace('/([,\.]00(\s)?('.CURRENCY_SIGN.')?)$/', '$2$3', money_format($format, $price));
    else
...
/***********************************************************************
 * get_top_nav_id() gets first level nav id
 * 
 * @param string $menu current menu
 *
 * @return integer
 */
function get_top_nav_id($menu)
{
    global $article_id;
    global $page_id;
    ...
    foreach($menu as $nav_id => $nav){
        ...
        if($nav_type == 'page' && $page_id == $item_id) $page_nav_id = $nav_id;
        if($article_id > 0 && $nav_type == 'article' && $article_id == $item_id) $article_nav_id = $nav_id;
    }
...
/***********************************************************************
 * has_child_nav() returns true if the current nav item has a submenu
 * 
 ...
 * @return boolean
 */
function has_child_nav($id_nav, $menu)
{
    foreach($menu as $nav_id => $nav){
...
/***********************************************************************
 * get_nav_url() gets the URL of the current nav item
 * 
...
function get_nav_url($nav)
{
    ...
    switch($nav['item_type']){
        case 'page':
            $href = DOCBASE.$pages[$nav['id_item']]['alias'];
            break;
        case 'article':
...
/***********************************************************************
 * get_translation() gets the translation of an entry in the admin language file
 * 
    ...
    if(strpos($entry, '[') !== false){
        if(preg_match_all('/\[(\w+)\]/', $entry, $matches) > 0){
            ...
            foreach($matches as $entry){
                if(isset($texts[$entry])) $value = str_replace('['.$entry.']', $texts[$entry], $value);
            }
        }
    }
    return $value;
...
/***********************************************************************
 * gm_strtotime() converts a date to GMT timestamp
 * 
 * @param string $date date to convert
 *
 * @return integer
 */
function gm_strtotime($date)
{
...
/***********************************************************************
 * gmtime() returns the current GMT timestamp
 *
 * @return integer
 */
function gmtime()
...
/***********************************************************************
 * getAltText() return the plural or singular variation of a text
 * 
 ...
 * @return string
 */
function getAltText($singular, $plural, $value, $mode = 'lc')
{
...
/***********************************************************************
 * getInitials() return the first letter of each word
 * 
 * @param string $input
 ...
 * @return string
 */
function getInitials($input, $limit = 3, $mode = 'uc')
{
    $wds = explode(' ', $input);
    $nb_wds = count($wds);
    $initials = '';
