...
function getBookedRooms($from_time, $to_time, $room_id = 0)
{
    ...
    $query_book = '
        SELECT stock, br.id_room as room_id, from_date, to_date, r.id_hotel
		...
		SELECT stock, r.id as room_id, from_date, to_date, r.id_hotel
		...
		SELECT (r.stock-rc.stock)+1 as stock, r.id as room_id, from_date, to_date, r.id_hotel
    ...
    if($result_book !== false){
        foreach($result_book as $i => $row){
            ...
            $res_hotel[$id_hotel][$id_room]['room_stock'] = max(0, $room_stock-$max);
        }
    }
    return $booked;
}
					...
					if($num_people > $people){
                        ...
                        if($extra_children > 0){
							for($j = 0; $j < $extra_children; $j++){
								...
								if($result_child_rate->execute() !== false && $db->last_row_count() > 0){
...
function getBookingSummary($room_id = null, $index = null, $amount = 0, $available = true)
{
    ...
    if($total > 0){
        ...
        $hotel_id = isset($_POST['id_hotel']) ? $_POST['id_hotel'] : 0;
        ...
        <button name="book" id="btn-book_'.$hotel_id.'" class="btn btn-success btn-lg btn-block mt5"><b>'.$texts['TOTAL'].' '.formatPrice($total*CURRENCY_RATE).'</b> <i class="fas fa-fw fa-hand-point-right"></i> '.$texts['BOOK'].'</button>';
